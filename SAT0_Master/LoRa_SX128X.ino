/**
    @file LoRaSX1280.ino
    @brief LoRa 2.4 utils functions for SATLLA0.

    This file contains LoRa 2.4 Radio functionality for the SATLLA0.
...
void lora_24_set_state(bool state)
{
    PRINTLN("Func: lora_24_set_state()");
    if (state)
    {
        lora_24_turn_on();
    }
...
void lora_24_turn_on()
{
    PRINTLN("Func: lora_24_turn_on()");
    digitalWrite(FAT_LORA_24_PIN, HIGH);
...
void lora_24_turn_off()
{
    PRINTLN("Func: lora_24_turn_off()");
...
void lora_24_resume()
{
    PRINTLN("Func: lora_24_resume()");
    
    if (!LT.checkDevice())
    {
        PRINTLN("Func: lora_24_resume()::modem failed");
        lora_24_setup();
    }
    else
    {
...
bool send_message_lora_24(uint8_t *message, uint8_t messageLength)
{
    ...
    if (!lora_24_is_on)
    {
        PRINTLN(F("LoRa SX1280: Sending Failed!"));
        PRINTLN(F("LoRa SX1280: Radio is off!"));
        return;
    }
    ...
    print_message(lora_24_tx_buf, messageLength);

    unsigned long start = millis();
    PRINT("Sending Time():\t");
    // uint8_t SX128XLT::transmit(uint8_t *txbuffer, uint8_t size, uint16_t timeout, int8_t txpower, uint8_t wait
    int state = LT.transmit(lora_24_tx_buf, messageLength, SECS_4, modem_lora_24_g.modem_power, false);
    PRINT(millis() - start);
    PRINTLN(" ms");
...
#endif
    // reply ack
    if (lora_data_g.msg_ack_req)
    {
        reply_ack_24(&lora_data_g);
    }
...
void wrap_message_24(uint8_t type)
{
    PRINTLN("Func: wrap_message_24()");

    return wrap_message_24(0x00, 0, type);
}

void wrap_message_24(uint8_t *message, uint8_t size, uint8_t type)
{
    PRINTLN("Func: wrap_message_24()");
    ...
    send_message_lora_24(wrap_msg_g.packet, LORA_HEADER_LENGTH + wrap_msg_g.msg_size);
}
...
void ranging_24_master_run(uint8_t destination)
{
    ...
    memcpy(rng_request_g.msg_payload, command_data_g.packet, COMMAND_HEADER_LENGTH + 1);
    rng_request_g.msg_size = COMMAND_HEADER_LENGTH + 1;

    PRINTLN(rng_request_g.msg_size);
...
void ranging_24_master_calibration()
{
