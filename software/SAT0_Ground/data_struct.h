/**
    @file data_struct.h
    @brief Data Struct and enums for SATLLA0 GS.

    This contains the data structure and the enum for the different messages for the SATLLA0 GS.
...
union lora_data_t
{
    struct
    {
        ...
        uint8_t msg_type;                      // 3, Message Type per message_type_e enum
...
union shrt_bcn_data_t
{
    ...
    uint8_t packet[SHRT_BCN_PACKET_LENGTH]; // This array is used to push out throught the LoRa module.
};
...
union gps_data_t
{
    ...
    uint8_t packet[GPS_PACKET_LENGTH]; // This array is used to push out through the LoRa module.
};
...
union sns_data_t
{
    struct
    {
        ...
        int8_t sns_ntc1;        // 8, Temperature
        int8_t sns_ntc2;        // 9, Temperature
        // int8_t sns_ntc3;       // 10, Temperature
        // uint16_t sns_mpu_acc;  // 11, Acceleration
        // uint16_t sns_mpu_gyro; // 12, Gyro
        // uint16_t sns_mpu_mag;  // 13, Mag
        // float    sns_mpu_temp_c;  // 14, MPU Temperature
        // uint16_t sns_bmp_alt_m; // 15, BMP Alt
    };
    uint8_t packet[SNS_PACKET_LENGTH]; // This array is used to push out throught the LoRa module.
};

//#define BEACON_PACKET_LENGTH (LORA_PACKET_LENGTH + GPS_PACKET_LENGTH)
//union beacon_data {
//  struct
//  {
//    lora_header_data beacon_header; // 1, Header
//    gps_data beacon_gps;            // 2, GPS
//    //mpu_data beacon_MPU;          // 3, MPU
//    //mission_data beacon_mission;  // 4, Mission
//  };
//  uint8_t packet[BEACON_PACKET_LENGTH]; // This array is used to push out through the LoRa module.
//};

// #define RPI_HEADER_LENGTH 4
// #define RPI_PACKET_LENGTH 204
// #define RPI_DATA_LENGTH (RPI_PACKET_LENGTH - RPI_HEADER_LENGTH)

// union rpi_data_t
// {
//     struct
//     {
//         uint8_t rpi_type;                     // 1, 0=meta, 1=photo, 2=txt 3:? ,9=None
//         uint8_t rpi_size;                     // 2, size.
//         uint8_t rpi_index;                    // 3, Index (Name).
//         uint8_t rpi_sub_index;                // 4, Sub Index (only for photo).
//         uint8_t rpi_payload[RPI_DATA_LENGTH]; // 5. The Data
//     };
//     uint8_t packet[RPI_PACKET_LENGTH]; // This array is used to push out through the LoRa module.
// };
...
union energy_data_t
{
    ...
    uint8_t packet[ENERGY_PACKET_LENGTH]; // This array is used to push out throught the LoRa module.
};
...
union ringbuffer_data_t
{
    ...
    uint8_t packet[RINGBUFFER_LENGTH]; // This array is used to push out throught the LoRa module.
};
...
union sd_data_t
{
    ...
    uint8_t packet[SD_DATA_LENGTH]; // This array is used to push out throught the LoRa module.
};
...
union modem_data_t
{
    struct
    {
        ...
        uint8_t modem_ds;         // 8, Gaussian shaping (FSK)
        uint32_t modem_sw;        // 9, Sync Word
        uint32_t modem_frequency; // 10, Frequency
...
#define GS_SETTING_PACKET_LENGTH 16

union gs_setting_data_t
{
    struct
    {
        uint8_t set_gs_crc;
        uint8_t set_local_address;
        uint8_t set_gs_gps_enable;
        uint8_t set_gs_drive_enable;
        float set_gs_lat;
        float set_gs_lon;
        uint8_t set_doppler_enable;
    };
    uint8_t packet[GS_SETTING_PACKET_LENGTH]; // This array is used to push out through the LoRa module.
};
